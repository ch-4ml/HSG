package com.hsg.intro.edu.hrd.controller;
...
public class EduHrdController {
	...
	// CEO 인사말 업데이트
	@RequestMapping(value="update.eh", method=RequestMethod.POST) // DI 의존성 주입
	public ModelAndView updateContents(Contents c, ModelAndView mv, HttpServletRequest request) {
		...
		System.out.println("in update.eh : " + c);
		
		try {
			cs.update(c);
			mv.setViewName("redirect:view.eh");
		} catch (ContentsException e) {
			mv.addObject("message",e.getMessage());
			mv.setViewName("common/errorPage");
		}		
